...
# Vector triad: 3 reads + 1 write = 4 doubles = 32 bytes, it would change for other operations. (24 for add, 16 for scale, and 16 for copy)
bytes_moved = 24 * df["problem_size"] * df["runs"]
...
plt.tight_layout()
plt.savefig("results/correct_add_throughput_vs_problem_size.png")
plt.close()
...
plt.grid(True, which="both")
plt.tight_layout()
plt.savefig("results/correct_add_bandwidth_vs_problem_size.png")
plt.close()
